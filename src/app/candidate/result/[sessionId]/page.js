...
export default async function ResultPage({ params }) {
  ...
  const sessions = await sql`
    SELECT ts.*, u.name as user_name, u.email as user_email, u.gender, u.birth_date, u.education
    FROM test_sessions ts 
    JOIN users u ON ts.user_id = u.id 
    WHERE ts.id = ${params.sessionId} LIMIT 1
  `;
  if (!sessions.length) notFound();
  
  const session = sessions[0];

  // Enforce access control: only HR or the candidate who owns the session can view
  if (user.role !== 'hr' && session.user_id !== user.id) {
    redirect('/candidate/dashboard');
  }

  // Candidates can only view completed tests
  if (user.role === 'candidate' && session.status !== 'completed') {
    redirect('/candidate/dashboard');
  }
  ...
  const gender = session.gender === 'female' ? 'Perempuan' : session.gender === 'male' ? 'Laki-laki' : 'Perempuan';
  const age = session.birth_date ? `${new Date().getFullYear() - new Date(session.birth_date).getFullYear()} Tahun` : '23 Tahun';
  const education = session.education || 'D4 / Sarjana Terapan';

  const iq = session.score || 100;
  let iqText = "";
  
  // Conditional narrative generation to hide numerical score for candidate
  if (isCandidate) {
    if (iq >= 120) {
      iqText = `Sdr/Sdri ${name} memiliki taraf intelegensi superior sehingga ia mampu untuk memahami instruksi yang kompleks, melakukan analisis data secara mendalam, serta memecahkan masalah-masalah berskala besar secara taktis dan terstruktur dengan sangat cepat. Ia memiliki kemampuan verbal yang prima dan pemahaman logikal di atas rata-rata kelompok usianya.`;
    } else if (iq >= 90) {
      iqText = `Sdr/Sdri ${name} memiliki taraf intelegensi rata-rata yang stabil sehingga ia mampu untuk memahami instruksi secara sederhana maupun menengah dengan baik. Ia membutuhkan waktu yang wajar untuk memahami detail instruksi yang terlalu rumit, namun memiliki dasar penalaran verbal dan numerikal yang mantap serta memadai dalam menyelesaikan beban kerja rutin harian.`;
    } else {
      iqText = `Sdr/Sdri ${name} lebih siap dalam memahami instruksi kerja operasional yang sifatnya praktis dan terstruktur. Ia memerlukan bimbingan bertahap untuk tugas yang memerlukan tingkat penalaran akademis yang tinggi, namun memiliki ketekunan operasional yang baik jika diberikan arahan kerja yang jelas.`;
    }
  } else {
    if (iq >= 120) {
  ...
  const personalityType = session.personality_type || detail.personality_type || "Advocate atau Kolaborator Mandiri yang berusaha membangun hubungan yang positif di tempat kerja.";
  ...
  return (
    <SidebarLayout user={user}>
      <div className="p-6 lg:p-8 max-w-5xl mx-auto">
        ...
        {/* Navigation Toolbar */}
        <div className="no-print mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white border border-ink-100 rounded-2xl p-4 shadow-sm">
          <div className="flex items-center gap-3">
            <Link 
              href={user.role === 'hr' ? `/hr/candidates/${session.user_id}` : '/candidate/dashboard'}
            ...
            <div>
              <h1 className="font-display text-base font-bold text-ink-900">Laporan Hasil Psikotes</h1>
              <p className="text-xs text-ink-400">Pratinjau Laporan Resmi A4</p>
            </div>
          </div>
          <PrintButton />
        </div>
        ...
        <div className="preview-container">
              ...
              <div className="cover-middle-box">
                <div className="cover-card">
                  ...
                  <div className="cover-card-row">
                    ...
                    <div className="cover-card-val">{name}</div>
                  </div>
                  <div className="cover-card-row">
                  ...
                  <div className="cover-card-row">
                    ...
                    <div className="cover-card-val">{session.position_applied || '-'}</div>
                  </div>
                  <div className="cover-card-row">
            ...
            <table className="identitas-table">
              <tbody>
                ...
                <tr>
                  ...
                  <td className="val-text">{session.position_applied || '-'}</td>
                </tr>
                <tr>
          ...
          <div className="a4-page">
            ...
            {/* Intelegensi range bar - showing category only if candidate */}
            <table className="iq-bar-table">
              <tbody>
                <tr>
                  <td className="iq-label">
                    {isCandidate ? 'Taraf Intelegensi' : `Intelegensi : ${iq}`}
                  </td>
                  <td className={iq < 80 ? 'active' : ''}>Sangat Rendah</td>
                  <td className={(iq >= 80 && iq < 90) ? 'active' : ''}>Rata-rata bawah</td>
                  <td className={(iq >= 90 && iq < 110) ? 'active' : ''}>Rata-Rata</td>
                  <td className={(iq >= 110 && iq < 120) ? 'active' : ''}>Rata-Rata Atas</td>
                  <td className={(iq >= 120 && iq < 130) ? 'active' : ''}>Tinggi</td>
                  <td className={iq >= 130 ? 'active' : ''}>Sangat Tinggi</td>
                </tr>
              </tbody>
            </table>
          ...
          {/* ================= PAGE 5 ================= */}
          <div className="a4-page page-no-break">
            <div className="header-logo">
              <div className="brand">Alpha Consulting</div>
              <div className="sub-brand">Psychological Service &amp; Business Consulting</div>
              <div className="contact">E-mail: alphaconsulting.id@gmail.com | HP: 082269899003</div>
