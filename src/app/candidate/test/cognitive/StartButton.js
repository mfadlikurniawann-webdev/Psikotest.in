...
export default function StartButton({ existing, module = 'cognitive' }) {
  ...
  const examUrl = `/candidate/test/${module}/exam`;
  ...
  return (
    <div className="flex flex-col items-center bg-ink-50 border border-ink-100 rounded-2xl p-6">
      ...
      {existing ? (
        ...
        <button onClick={() => { setLoading(true); router.push(examUrl); }}
