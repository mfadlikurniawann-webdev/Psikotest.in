...
export default function ExamTaker({ session, questions }) {
  ...
  // Adjust timer from session start
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const snap = setInterval(captureSnapshot, 20000);
    return () => { clearInterval(snap); streamRef.current?.getTracks().forEach(t => t.stop()); };
  }, [disq]);
  ...
  function captureSnapshot() {
    ...
    fetch('/api/proctor/snapshot', {
      ...
      body: JSON.stringify({ sessionId: session.id, image: canvas.toDataURL('image/jpeg', 0.5) })
    });
  }
  ...
  async function doSubmit(forced = false) {
    ...
    const res = await fetch('/api/test/submit', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ sessionId: session.id, answers, duration }) });
    const data = await res.json();
  ...
  const getCat  = i => {
    if (i < 25) return { stage:'Tahap 1: Kemampuan Verbal', label:'Verbal', range:'Soal 1–25' };
    if (i < 50) return { stage:'Tahap 2: Kemampuan Numerik', label:'Numerik', range:'Soal 26–50' };
    if (i < 75) return { stage:'Tahap 3: Penalaran Logika & Spasial', label:'Logika', range:'Soal 51–75' };
    return { stage:'Tahap 4: Karakteristik Kepribadian Kerja', label:'Kepribadian', range:'Soal 76–100' };
  };
  ...
  const stages = [
    { label:'Verbal (1–25)',    range:[0,25]  },
    { label:'Numerik (26–50)',  range:[25,50] },
    { label:'Logika (51–75)',   range:[50,75] },
    { label:'Kepribadian (76–100)', range:[75,100] },
  ];
    ...
    <div className="bg-ink-50 min-h-screen" style={{userSelect:'none',WebkitUserSelect:'none'}} onContextMenu={e=>e.preventDefault()}>
      ...
      <header className="fixed top-0 left-0 right-0 z-30 bg-white border-b border-ink-100 px-4 md:px-6 h-14 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0"><p className="font-semibold text-ink-900 text-sm truncate">Psikotes Terpadu</p><p className="text-ink-400 text-xs">Psikotest.in</p></div>
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-20 bg-white border-t border-ink-100 shadow-xl py-3.5 px-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm font-semibold text-ink-900">Terjawab: <span className="text-gold font-bold">{answered}</span>/100 soal</p>
        </div>
      ...
      {showWarn && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl p-7 max-w-sm w-full mx-4 text-center shadow-2xl border border-ink-100">
            ...
            <button onClick={closeWarn} className="w-full bg-ink-900 hover:bg-ink-700 text-white py-2.5 rounded-xl text-sm font-semibold transition">
              Kembali ke Tes
            </button>
          </div>
        </div>
      ...
      {showSubmit && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl p-7 max-w-sm w-full mx-4 text-center shadow-2xl border border-ink-100">
            ...
            <p className="text-ink-400 text-sm mb-2">Sudah terjawab: <span className="font-bold text-ink-900">{answered}</span>/100 soal.</p>
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowSub(false)} className="flex-1 border border-ink-200 text-ink-700 py-2.5 rounded-xl text-sm font-semibold hover:bg-ink-50 transition">
                Kembali
              </button>
